...
from tito.common import error_out, debug, run_command, get_spec_version_and_release, find_spec_like_file, \
                        get_relative_project_dir, get_relative_project_dir_cwd
...
class GitAnnexSpecBuilder(GitAnnexBuilder):
    ...
    def _setup_sources(self):
        ...
        debug('SETUP SOURCES')
        working_path = os.getcwd() if self.relative_project_dir in os.path.join(os.getcwd(), '') else os.path.join(os.getcwd(), self.relative_project_dir)
    ...
    def tgz(self):
        ...
        self.sources = [os.path.join(self.rpmbuild_gitcopy, source) for source in result.split("\n")]
